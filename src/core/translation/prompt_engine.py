"""
Prompt Engine - Advanced prompt generation for translation
Handles constraints, context, terminology, and model-specific optimizations.
"""
...
class PromptOptimizer:
    ...
    def _add_structure_markers(self, prompt: str) -> str:
        ...
        # Add clear section markers for Gemini
        prompt = prompt.replace("CRITICAL CONSTRAINTS:", "=== CRITICAL CONSTRAINTS ===")
        prompt = prompt.replace("CONTEXT:", "=== CONTEXT ===")
        prompt = prompt.replace("TERMINOLOGY GUIDELINES:", "=== TERMINOLOGY GUIDELINES ===")
        prompt = prompt.replace("SPECIAL INSTRUCTIONS:", "=== SPECIAL INSTRUCTIONS ===")
        prompt = prompt.replace("TEXT TO TRANSLATE:", "=== TEXT TO TRANSLATE ===")
        prompt = prompt.replace("OUTPUT FORMAT:", "=== OUTPUT FORMAT ===")
        return prompt

    def _expand_context(self, prompt: str) -> str:
        """Expand context for Claude"""
        # Claude appreciates more detailed context
        # For now, just return as-is (could add more context expansion logic)
        return prompt

    def _add_examples(self, prompt: str) -> str:
        """Add few-shot examples for Llama"""
        # Llama benefits from examples
        # For now, just return as-is (could add example translations)
        return prompt
