#!/usr/bin/env python3
"""
Text Length Controller (Instruction 14)
...
class FontMetrics:
    ...
    def __init__(self, font_data: bytes):
        ...
        if not FONTTOOLS_AVAILABLE:
            raise ImportError("fonttools is required for precise font metrics")
...
class TextLengthController:
    ...
    def _get_font_metrics(self, font_info: Dict) -> Optional[FontMetrics]:
        ...
        # Load font
        if font_info.get('data') and FONTTOOLS_AVAILABLE:
            try:
                ...
                logger.warning(f"Failed to load font metrics: {e}")
    ...
    def fit_translation(self, translation: str, source_bbox: Dict,
        ...
        overflow_ratio = max(
            measurement.width / bbox_width if bbox_width > 0 else 1.0,
            measurement.height / bbox_height if bbox_height > 0 else 1.0
        )
        ...
        if overflow_ratio > 1.0:
            ...
            for strategy_name, strategy_func in strategies:
                ...
                new_overflow = max(
                    new_measurement.width / bbox_width if bbox_width > 0 else 1.0,
                    new_measurement.height / bbox_height if bbox_height > 0 else 1.0
                )
    ...
    def calculate_max_length(self, source_text: str, source_bbox: Dict,
        ...
        max_chars = int(max_width / avg_char_width) if avg_char_width > 0 else len(source_text)
    ...
    def validate_translation_fit(self, translation: str, source_bbox: Dict,
        ...
        width_ratio = measurement.width / bbox_width if bbox_width > 0 else 1.0
        height_ratio = measurement.height / bbox_height if bbox_height > 0 else 1.0
