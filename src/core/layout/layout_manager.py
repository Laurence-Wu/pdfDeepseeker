...
try:
    ...
    logging.warning("sklearn not available, column detection will use fallback method")
...
class LayoutManager:
    ...
    def _detect_columns(self, elements: List[LayoutElement]) -> List[Dict]:
        ...
        if len(x_coords) > 1:
            if SKLEARN_AVAILABLE:
                ...
                labels = self._simple_column_clustering(x_coords)
    ...
    def _simple_column_clustering(self, x_coords: np.ndarray, threshold: float = 50) -> np.ndarray:
        """Simple fallback for column clustering without sklearn"""
        if len(x_coords) == 0:
            return np.array([])

        # Sort coordinates
        sorted_indices = np.argsort(x_coords.flatten())
        sorted_coords = x_coords[sorted_indices]

        # Assign labels based on gaps
        labels = np.zeros(len(x_coords), dtype=int)
        current_label = 0
            ...
            for j, elem2 in enumerate(elements[i+1:], i+1):
                ...
                if self._is_caption(elem1, elem2):
                    relationships.append({
                        ...
                        'image': elem1.id if elem1.type in ['image', 'table'] else elem2.id,
                        'caption': elem2.id if elem1.type in ['image', 'table'] else elem1.id
                    })
                ...
                if self._is_side_by_side(elem1, elem2):
                    relationships.append({
                        ...
                        'left': elem1.id if elem1.bbox.get('x', 0) < elem2.bbox.get('x', 0) else elem2.id,
                        'right': elem2.id if elem1.bbox.get('x', 0) < elem2.bbox.get('x', 0) else elem1.id
                    })
    ...
    def _is_text_wrapping(self, elem1: LayoutElement, elem2: LayoutElement) -> bool:
        ...
        if distance < self.wrap_distance:
            ...
            text_x_range = (text_elem.bbox.get('x', 0), text_elem.bbox.get('x', 0) + text_elem.bbox.get('width', 0))
            img_x_range = (img_elem.bbox.get('x', 0), img_elem.bbox.get('x', 0) + img_elem.bbox.get('width', 0))
    ...
    def _is_caption(self, elem1: LayoutElement, elem2: LayoutElement) -> bool:
        ...
        text_center_x = text_elem.bbox.get('x', 0) + text_elem.bbox.get('width', 0) / 2
        visual_center_x = visual_elem.bbox.get('x', 0) + visual_elem.bbox.get('width', 0) / 2

        # Check horizontal alignment
        if abs(text_center_x - visual_center_x) < visual_elem.bbox.get('width', 0) / 2:
            # Check vertical proximity
            vertical_distance = abs(text_elem.bbox.get('y', 0) -
                                  (visual_elem.bbox.get('y', 0) + visual_elem.bbox.get('height', 0)))
    ...
    def _is_side_by_side(self, elem1: LayoutElement, elem2: LayoutElement) -> bool:
        ...
        y1_range = (elem1.bbox.get('y', 0), elem1.bbox.get('y', 0) + elem1.bbox.get('height', 0))
        y2_range = (elem2.bbox.get('y', 0), elem2.bbox.get('y', 0) + elem2.bbox.get('height', 0))
        ...
        if overlap_end > overlap_start:
            ...
            if vertical_overlap / min_height > 0.5:
                ...
                x1_end = elem1.bbox.get('x', 0) + elem1.bbox.get('width', 0)
                x2_start = elem2.bbox.get('x', 0)
                x2_end = elem2.bbox.get('x', 0) + elem2.bbox.get('width', 0)
                x1_start = elem1.bbox.get('x', 0)
    ...
    def _get_wrap_side(self, text: LayoutElement, img: LayoutElement) -> str:
        ...
        if text.bbox.get('x', 0) < img.bbox.get('x', 0):
            return 'left'
        elif text.bbox.get('x', 0) > img.bbox.get('x', 0) + img.bbox.get('width', 0):
            return 'right'
        elif text.bbox.get('y', 0) < img.bbox.get('y', 0):
            return 'top'
        else:
            return 'bottom'
    ...
    def _detect_alignment(self, elements: List[LayoutElement]) -> str:
        ...
        x_positions = [e.bbox.get('x', 0) for e in elements]
    ...
    def _is_form_layout(self, elements: List[LayoutElement]) -> bool:
        ...
        x_positions = [e.bbox.get('x', 0) for e in text_elements]
        y_positions = [e.bbox.get('y', 0) for e in text_elements]
    ...
    def _detect_margin_notes(self, elements: List[LayoutElement]) -> bool:
        ...
        for elem in text_elements:
            ...
            if elem.bbox.get('x', 0) < 50 or elem.bbox.get('x', 0) > 500:  # Adjust based on page size
    ...
    def _calculate_overlay_adjustment(self, text: Dict, img: Dict) -> Dict:
        ...
        return {
            ...
            'action': 'reposition',
            'new_position': {
                'x': img['bbox'].get('x', 0) + img['bbox'].get('width', 0) + 10,
                'y': text['bbox'].get('y', 0)
            }
        }
            ...
            if rel['type'] == LayoutRelationship.CAPTION.value:
                ...
                if rel.get('caption') == element['id']:
                    ...
                    if img_elem:
                        ...
                        adjusted['bbox']['y'] = (img_elem['bbox'].get('y', 0) +
                                                img_elem['bbox'].get('height', 0) + 10)
                        # Center horizontally
                        adjusted['bbox']['x'] = img_elem['bbox'].get('x', 0)
