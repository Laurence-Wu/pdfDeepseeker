"""
Edge Case Handler - Comprehensive handler for document formatting edge cases
...
class EdgeCaseHandler:
    ...
    def __init__(self, config: Dict = None):
        ...
        # Handler mapping
        self.handlers = {
            'rotated_text': self.handle_rotated_text,
            'vertical_text': self.handle_vertical_text,
            'multi_column': self.handle_multi_column,
            'footnotes': self.handle_footnotes,
            'drop_caps': self.handle_drop_caps,
            'form_fields': self.handle_form_fields,
            'annotations': self.handle_annotations,
            'hyperlinks': self.handle_hyperlinks,
            'page_numbers': self.handle_page_numbers,
            'text_in_shapes': self.handle_text_in_shapes
        }
            ...
            if transform and len(transform) >= 2:
                ...
                if abs(angle) > 5:  # More than 5 degrees
                    edge_cases.append(EdgeCase(
                        ...
                        element_id=element.get('id', f"elem_{len(edge_cases)}"),
                        confidence=0.95,
    ...
    def _detect_vertical_text(self, page_data: Dict) -> List[EdgeCase]:
        ...
        for element in page_data.get('text_elements', []):
            ...
            if len(chars) > 1:
                ...
                y_positions = [c.get('y', 0) for c in chars if 'y' in c]
                x_positions = [c.get('x', 0) for c in chars if 'x' in c]
    ...
    def _detect_multi_column(self, page_data: Dict) -> List[EdgeCase]:
        ...
        if len(text_blocks) > 5:
            # Cluster x-positions
            x_positions = [block.get('bbox', {}).get('x', 0) for block in text_blocks if 'bbox' in block]
        ...
        for element in page_data.get('text_elements', []):
            ...
            if (y > page_height * 0.8 and
                ...
                edge_cases.append(EdgeCase(
                    ...
                    element_id=element.get('id', f"fn_{len(edge_cases)}"),
                    confidence=0.8,
                    metadata={'marker': text[0] if text else '', 'font_size': font_size},
                ...
                if first_char:
                    ...
                    if char_size > avg_size * 2.5:
                        edge_cases.append(EdgeCase(
                            ...
                            element_id=block.get('id', f"block_{len(edge_cases)}"),
                            confidence=0.9,
                            metadata={'char': first_char.get('text', ''), 'size_ratio': char_size / avg_size},
    ...
    def _detect_form_fields(self, page_data: Dict) -> List[EdgeCase]:
        ...
        for field in page_data.get('form_fields', []):
            edge_cases.append(EdgeCase(
                ...
                element_id=field.get('id', f"field_{len(edge_cases)}"),
                confidence=1.0,
                metadata=field,
        ...
        for element in page_data.get('text_elements', []):
            ...
            if ((y > page_height * 0.9 or y < page_height * 0.1) and
                ...
                edge_cases.append(EdgeCase(
                    ...
                    element_id=element.get('id', f"pgnum_{len(edge_cases)}"),
                    confidence=0.7,
    ...
    def _detect_hyperlinks(self, page_data: Dict) -> List[EdgeCase]:
        ...
        for link in page_data.get('links', []):
            edge_cases.append(EdgeCase(
                ...
                element_id=link.get('id', f"link_{len(edge_cases)}"),
                confidence=1.0,
                metadata=link,
    ...
    def apply_strategies(self, page_data: Dict, edge_cases: List[EdgeCase]) -> Dict:
        ...
        for edge_case in edge_cases:
            # Get handler
            handler = self.handlers.get(edge_case.type)
            if handler:
                # Find element
    ...
    def _find_element(self, page_data: Dict, element_id: str) -> Optional[Dict]:
        ...
        # Return page data for page-level edge cases
        if element_id == 'page':
            return page_data
